
# Countdown - Create a function that accepts a number as an input. Return a new list that counts down by one, from the number (as the 0th element) down to 0 (as the last element).
list = []
def countDown(i):
    while i >= 0:
        list.append(i)
        i=i-1
    print(list)
countDown(50)



#Print and Return - Create a function that will receive a list with two numbers. Print the first value and return the second. Example: print_and_return([1,2]) should print 1 and return 2
def print_and_return(num_list):
    print(num_list[0])
    return(num_list[1])
x = print_and_return([1,3])
print(x) #THIS LINE SHOWS THAT X NOW EQUALS NUM_LIST[1] AND WHAT IS BEING RETURNED



#First Plus Length - Create a function that accepts a list and returns the sum of the first value in the list plus the list's length. Example: first_plus_length([1,2,3,4,5]) should return 6 (first value: 1 + length: 5)
def first_plus_length(num_list):
    j = num_list[0] + len(num_list)
    return j
x = first_plus_length([2,4,6,8])
print(x) # x is equal to the return value of the function which is the value of j



#Values Greater than Second - Write a function that accepts a list and creates a new list containing only the values from the original list that are greater than its 2nd value. Print how many values this is and then return the new list. If the list has less than 2 elements, have the function return False
#Example: values_greater_than_second([5,2,3,2,1,4]) should print 3 and return [5,3,4]
#Example: values_greater_than_second([3]) should return False
new_list = []
def values_greater_than_second(list):
    if len(list) < 2:
        return False
    for i in range(0, len(list)):
        if list[i]>=list[1]:
            new_list.append(list[i])
    print(len(new_list))
    return new_list

print(values_greater_than_second([1,3,5,9,11]))



#This Length, That Value - Write a function that accepts two integers as parameters: size and value. The function should create and return a list whose length is equal to the given size, and whose values are all the given value.
#Example: length_and_value(4,7) should return [7,7,7,7]
#Example: length_and_value(6,2) should return [2,2,2,2,2,2]
new_list = []
def length_and_value(size, value):
    for i in range(0, size):
        new_list.append(value)
    return new_list

print(length_and_value(5,9))
